    ...
    , ui(new Ui::MainWindow)
{
    ...
    bruno->setPos(400,250); // Establece la posición inicial de Bruno en la escena
    ...
    ui->lcdNumber_2->display(vidaE1);
    ui->lcdNumber->display(vidaJ);

    while(vidaJ>0 && vidaE1>0){

        if (QAT == 0 && (ev->key()==Qt::Key_P)){
            col = 0;
            escena->addItem(balo);
        ...
        if (QAT == 1){
            ...
            amplitude = 180;                     // Ajusta la amplitud según sea necesario
...
void MainWindow::movimientorecto(){
    ...
    qreal x = start1.x() + 0.2*time1; //amplitude * qSin(2 * M_PI * time / period); // Calcula la nueva posición x de Bruno usando la función seno

    // Establecer la nueva posición de Bruno en la escena
    balo->setPos(x, start1.y());

    if (balo->x()>550){
        escena->removeItem(balo);
        //delete balo;
    }
